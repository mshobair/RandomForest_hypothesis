...
def shuffle_df(n):
    ...
    seed = int(initial_seed/1000000000)
...
df_n = 11 # could be any value outside of accepted range
while  0 > df_n or 9 < df_n:
    try:
...
def split_prep(train_frac):
    ...
    # training
    training = []
    training_Y = np.array(input_shuffled_bool.iloc[test_end : , 0])
    training_X = np.array(input_shuffled_bool.iloc[test_end : , 1:])
...
#   print predictive performance: balanced accuracy and specifically True-label prediction
    ...
    Balanced_accuracy_CV = Balanced_accuracy_CV + BA
    PA = tp/(tp+fn)
    Pos_accuracy_CV = Pos_accuracy_CV + PA
